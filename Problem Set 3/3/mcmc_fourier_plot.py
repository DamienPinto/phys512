...
def get_fourier_plots(mcmc_data, x):
	...
	for i in range(len(title_arr)):
		...
		fourier_mcmc = np.abs(np.fft.rfft(mcmc_data[:, i][int(len(mcmc_data[:,i])/2):]))

		plt.clf()
		plt.title(title)
		plt.xlabel(r"$k[\lambda \cdot Mpc^{-1}]$")
		plt.ylabel("Insert Correct Units Here")
		plt.plot(fourier_mcmc)
...
def main():
	mcmc_data = get_mcmc_data("10000_mcmc_chain_cov_restr_tau0.05_scl0.4.txt")
