...
def get_3d_cyl_w_bump(shape,r,l,pos,drctn=0):
    ...
    cyl = np.where(np.logical_or(np.logical_and(perp_norm<=r,par_norm<=l),np.logical_and(perp_norm<=(1.1*r),par_norm<=(l/20))),ones,zeros)
    cyl_mask =  np.where(np.logical_or(np.logical_and(perp_norm<=r,par_norm<=l),np.logical_and(perp_norm<=(1.1*r),par_norm<=(l/20))),True,False)
