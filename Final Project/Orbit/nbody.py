...
class Space:
    ...
    def __init__(self,L=[100,100,100],dx=[1,1,1],dt=1,n=1,init_type=0,m_dist=None,pos_dist=None,v_dist=None,fuzz_dist=1):
        ...
        #Function for rest of numbers. Nedd to compute situations corresponding to "stable" orbit.
            ...
            print("Initializing particles in psace according to some specified distribution")
...
def evolveCFS(universe,pbc,get_forces,get_pot,pot_filter):
    ...
    a_i = forces[pos_idx_x_i[:,0],pos_idx_x_i[:,1],pos_idx_x_i[:,2]]/np.repeat(ms.T,3,axis=0).reshape(len(ms),3)/L

    #Get energy of system bc it's convenient
    E_pot = np.sum(-ms*pot[pos_idx_x_i[:,0],pos_idx_x_i[:,1],pos_idx_x_i[:,2]])
    E_kin = np.sum(0.5*ms*np.sum(v_i**2,axis=1))
    [E_pot,E_kin] = round_floats([E_pot,E_kin])
...
def main():
    if len(sys.argv) != 1:
        ...
        n = 2
        #       L,dx,dt,n,init_type 
        uni_params = [np.array([L,L,L]),np.array([dx,dx,dx]),dt,n,2]
        fuzzy_dist = 1
        niter = 100
        pbc = 0
