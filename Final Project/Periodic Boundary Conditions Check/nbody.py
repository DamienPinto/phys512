...
def evolve(universe,fuzz_dist,niter,pbc):
    ...
    for i in range(niter):
        ...
        plt.savefig("PerBoundCdtns Test Images X-axis/pbc_snap_" + str(i) + ".png")
        plt.pause(1e-3)
        cb1.remove()
        cb2.remove()
...
def main():
    if len(sys.argv) != 1:
        ...
        #       L,dx,dt,n,init_type 
        uni_params = [np.array([L,L,L]),np.array([dx,dx,dx]),dt,n,1]
        fuzzy_dist = 1
        niter = 10
        pbc = 1
